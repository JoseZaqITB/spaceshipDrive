import TheDriving from "./TheDriving"
import FinalDestination from "./FinalDestination";
import useGame from "../stores/useGame"
/* import { Perf } from "r3f-perf"; */
import { useEffect, useState } from "react";
import { globals } from "../utils";
import { useKeyboardControls } from "@react-three/drei";

export default function SceneManager() {
    const scene = useGame((state) => state.scene); 
    const [transition, setTransition] = useState(false);

    
    // handle keyboards 
      const [subscribeKeyboards] = useKeyboardControls();
      const setButtons = useGame((state) => state.setButtons);
      useEffect(()=> {
        const keyboardUnsub = subscribeKeyboards((state) => {setButtons({powerUp: state.powerUp, interact: state.interact})});
        // listen to scene changes
        const unsubscribeScene = useGame.subscribe((state) => state.scene ,() => {
                setTransition(true)
                const timer = setTimeout(() => {
                    setTransition(false);
                },globals.transitionDelay * 1000 -500);
                return () => {
                    clearTimeout(timer);
                }
        });

        return () => {
            keyboardUnsub();
            unsubscribeScene();
        }
    },[]);  
    
    if(transition) return null;
    return <>
        {/* <Perf position="top-left" /> */}
        {scene === "finalDestination" && <FinalDestination />} 
        {scene === "theDriving" &&  <TheDriving />}
    </>;
}